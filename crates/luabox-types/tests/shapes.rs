...
// === `---@class Impl : Interface` conformance (#107) =======================
//
// Pure LuaCATS: no `.luab` files, no ambient defs. luals declares `: Shape`
// but trusts it; luabox verifies the carrier actually provides the
// interface's members with compatible signatures, `__index`-aware so
// classic inheritance is not wrongly flagged.

/// Check a plain LuaCATS source (no shapes, no ambient defs) at strict.
fn check_plain(source: &str) -> Vec<Diagnostic> {
    let parsed = parse(source, Dialect::Lua54);
    assert_eq!(parsed.errors(), &[], "fixture must parse cleanly");
    luabox_types::check_file(&parsed, "src/main.lua", Strictness::Strict)
}
