...
Owner: Starscream (Seekers lead). Tracker: GitLab issues on `origin` (decision 14).
Sprint state: root `STATUS.md` + the current card on `boards/improvement-sprint/`.

## M0 — Adoption (2026-09-06 →)

Sign the repo off as run to the Seekers' standard.
...
- Styleguide/architecture audit (Soundwave) → findings filed as issues.
...
- Soundwave audit findings from M0 that are structural rather than local:
