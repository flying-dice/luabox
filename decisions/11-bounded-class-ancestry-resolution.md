...
Resolving a class walks its ancestor chain recursively, one native stack frame per link, so the survivable depth is a property of the thread the walk happens to run on. Measured on the round-5 tree: ~45,500 links on the CLI's 16 MiB pinned thread, ~7,900 in the debug profile CI uses, and ~990 on an un-pinned default-stack thread — the last being what an embedder calling `luabox_lsp::run_stdio` directly gets, since that path pins nothing. Past the limit the process aborts: no diagnostic, no file name, no line.
